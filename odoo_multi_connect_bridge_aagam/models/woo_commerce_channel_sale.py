...
class WooCommerceChannelSale(models.Model):
    ...
    def change_woo_comm_feed_cron(self):
        ...
        self.env.ref('odoo_multi_connect_bridge_aagam.cron_import_product').write(vals)
        self.env.ref('odoo_multi_connect_bridge_aagam.cron_import_category').write(vals)
        self.env.ref('odoo_multi_connect_bridge_aagam.cron_import_partner').write(vals)
    ...
    def create_woo_comm_product(self, template, woocommerce):
        if template:
            ...
            #     print("if: product_dict['images':", product_dict['images'])
            if template.length or template.width or template.height:
                dimensions = {
                    u'width': str(template.width) or "",
                    u'length': str(template.length) or "",
    ...
    def create_woo_comm_product_varients(self, woocommerce, product_id, variation_ids):
        variant_list = []
        attribute_list = []
        varient_image = False
        print("-------test_woo_comm_product_varient")
        for variant_id in variation_ids:
            variant = woocommerce.get('products/' + str(product_id) + "/variations/" + str(variant_id)).json()
            # variant = woocommerce.get('products/' + str(product_id) + "/variations/" + str(variant_id)).json()
            print("--varients",variant)
            if variant['attributes']:
                ...
                for attributes in variant['attributes']:
                    ...
                    attrib_value_id = self.env['woo.comm.attribute.value.mapping'].search(
                        [('woo_comm_attribute_value_name', '=', attributes['option']), ('channel_id.id', '=', self.id),
                    ...
                    attr['attrib_value_id'] = attrib_value_id.woo_comm_attribute_value_id
                    attribute_list.append(attr)
                    print("--attribute_list--",attribute_list)
                    print("--image--",variant['image'])
                    ...
                    if product_data:
                        ...
                        for product in product_data:
                            ...
                            if not self.env['template.mapping'].search(
                                ...
                                if product['type'] == 'variable':
                                    print("--------test")
                                    variants = self.create_woo_comm_product_varients(woocommerce, product['id'], product['variations'])
                                    print("---varients",variants)
                                count = count + 1
                                for category in product['categories']:
                                    category_id = self.env['category.feed'].search(
                                        [('name', '=', category), ('channel_id.id', '=', self.id)])
                                    print("---categ",category_id)
    ...
    def update_woo_comm_products(self, woocommerce=False):
            ...
            if 'message' in product_data:
                ...
                for product in product_data:
                    ...
                    if update_record:
                        count += 1
                        update_record.state = 'update'
                        #print("2222222222",update_record.state)

                        if product['type'] == 'variable':
                            update_record.write({'feed_variants': [(5,), ]})
                            #print("&&&&&__________",update_record)
                            variants = self.create_woo_comm_product_varients(woocommerce, product['id'],
                                                                        product['variations'])
                        for category in product['categories']:
                            category_id = self.env['category.feed'].search(
                        ...
                        #print("^^^^^^^^^^^^^^^",update_record,product_feed_dict)
                        #print("**************self",self.id)
        ...
        # print("^^^^^^^^^^^^^^^^^^^^^^^^^^^6",self.update_product_date)
