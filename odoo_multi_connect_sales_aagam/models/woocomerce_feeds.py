...
class ProductFeed(models.Model):
    ...
    def _create_product_line(self, variant, template_id, store_id, location_id,channel_id):
        print("----varient11111",variant)
        print("----5555555",template_id)
        print("----66666666",store_id)
        print("----77777777",location_id)
        print("----8888888",channel_id)
        ...
        if not exists:
            ...
            if not product_exists_odoo:
                ...
                print("--product_id11111111111111111111--",product_product_id)
                vals.pop('message_follower_ids','')
                status = True
                ...
                print("---------mapping id else------", mapping_id)
        else:
            mapping_id = exists
            print("-==== ==== =====")
        return message
    ...
    def _create_product_lines(self, variant_ids, template_id, store_id, location_id,channel_id):
        message = ''
        print("----varient",variant_ids)
        print("----111111",template_id)
        print("----22222222",store_id)
        print("----33333333333333",location_id)
        print("----444444444444444",channel_id)

        for variant_id in self.env['product.variant.feed'].browse(variant_ids):
            print("----varient",variant_id)
            message += self._create_product_line(
                variant_id, template_id, store_id, location_id,channel_id)

            print("-------messa5555555555555555ge",message)
        return message

    @api.model
    def change_product_price(self, product_id, price, channel_id):
        print("#############",product_id,price,channel_id)
        ...
        print("@@@@@@@@@@@@@@@@@@@pricelist_item_0",pricelist_item)
        if pricelist_item:
            print("))))))))))))))))))))))))")
            pricelist_item = pricelist_item[0]
            print("pricelist_item_1",pricelist_item)
            pricelist_item.write(dict(fixed_price=price))
            print("pricelist_item_2",pricelist_item)
            ...
            print("$$$$$$$$$$product_create",pricelist_item)

    @api.model
    def change_product_qty(self, product_id, qty_available, location_id, template_id):
        print("Inside : change_product_qty:", product_id, qty_available, location_id, product_id.type, template_id)
            ...
            if match:
                ...
                if len(variant_lines):
                    ...
                    for variant_id in template_id.product_variant_ids:
                        ...
                        print("-----match_product",match_product)
                ...
                try:
                    ...
                    if len(variant_lines) and template_id:
                        ...
                        res = self._create_product_lines(
                            variant_lines, template_id, store_id, location_id,channel_id)
                        print("-----rrrrrrrrrrrrrrrrrrrrrrrrrrres",res)
                        if res:
                            res += message
                            state = 'error'
                        ...
                        for variant_id in template_id.product_variant_ids:
                            variant_vals = variant_id.read(['default_code','barcode'])[0]
                            print("111111111111111111",variant_vals)
                            abcd = self.change_product_price(
                                product_id =variant_id,
                                price = list_price,
                                channel_id = channel_id
                            )
                            print("2222222222222222222222222",abcd)
